...
SELECT *
FROM Employees
WHERE (LENGTH(CONCAT(FirstName, LastName)) - 
...
SELECT 
    ...
    SUM(CASE WHEN DATEDIFF(CURDATE(), HireDate) > 1095 THEN 1 ELSE 0 END) AS Over3Years,
    (SUM(CASE WHEN DATEDIFF(CURDATE(), HireDate) > 1095 THEN 1 ELSE 0 END) * 100.0 / COUNT(*)) AS PercentOver3Years
FROM Employees
...
SELECT JobDescription, SpacemanID, ExperienceYears
FROM Personal p
...
1. Write an SQL query that separates the uppercase letters, lowercase letters, numbers, and other characters from the given string 'tf56sd#%OqH' into separate columns.
  WITH chars AS (
  SELECT SUBSTRING('tf56sd#%OqH', n, 1) AS ch
  FROM (
...
SELECT 
    GROUP_CONCAT(CASE WHEN ch REGEXP '[A-Z]' THEN ch END) AS Uppercase,
    GROUP_CONCAT(CASE WHEN ch REGEXP '[a-z]' THEN ch END) AS Lowercase,
    GROUP_CONCAT(CASE WHEN ch REGEXP '[0-9]' THEN ch END) AS Numbers,
    GROUP_CONCAT(CASE WHEN ch REGEXP '[^A-Za-z0-9]' THEN ch END) AS Others
FROM chars;
...
3. You are given the following table, which contains a VARCHAR column that contains mathematical equations. Sum the equations and provide the answers in the output.(Equations)
  SELECT Equation, 
       (SELECT SUM(eval_result) 
        FROM (
           SELECT (2+3) AS eval_result UNION
           SELECT (10-4) UNION
           SELECT (5*6)
        ) t) AS TotalSum
FROM Equations;
