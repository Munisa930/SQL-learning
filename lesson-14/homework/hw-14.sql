...
SELECT 
    @str AS Original,
    TRANSLATE(@str,'0123456789','XXXXXXXXXX') AS Replaced;
...
SELECT 
    @str AS Original,
    REPLACE(@str, TRANSLATE(@str,'abcdefghijklmnopqrstuvwxyz','..........................'), '') AS Numbers,
    REPLACE(@str, TRANSLATE(@str,'0123456789','..........'), '') AS Letters;
...
WITH cte AS (
    SELECT 1 AS n
    UNION ALL SELECT n+1 FROM cte WHERE n < LEN(@str)
)
...
1. In this puzzle you have to swap the first two letters of the comma separated string.(MultipleVals)
  SELECT Id,
       STUFF(Vals,1,1,RIGHT(LEFT(Vals,2),1)) AS Swapped
FROM MultipleVals;
...
WITH WeeklyTotals AS (
    SELECT FinancialWeek, FinancialYear,
           SUM(ISNULL(SalesLocal,0)+ISNULL(SalesRemote,0)) AS TotalSales
    FROM WeekPercentagePuzzle
    GROUP BY FinancialWeek, FinancialYear
),
DayAreaSales AS (
    SELECT w.Area, w.[Date], w.FinancialWeek, w.FinancialYear,
           (ISNULL(SalesLocal,0)+ISNULL(SalesRemote,0)) AS DailySales
    FROM WeekPercentagePuzzle w
)
SELECT d.Area, d.[Date], d.FinancialWeek, d.FinancialYear,
       d.DailySales,
       (100.0*d.DailySales/t.TotalSales) AS PercentageOfWeek
FROM DayAreaSales d
JOIN WeeklyTotals t 
  ON d.FinancialWeek = t.FinancialWeek AND d.FinancialYear = t.FinancialYear;
