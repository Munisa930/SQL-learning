...
8.  Write a query to compare the current sale amount with the next sale amount in terms of percentage change.
  SELECT 
    ProductName, SaleDate, SaleAmount,
    ((LEAD(SaleAmount) OVER (ORDER BY SaleDate) - SaleAmount) * 100.0 / SaleAmount) AS PercChangeToNext
FROM ProductSales;
  
9. Write a query to calculate the ratio of the current sale amount to the previous sale amount within the same product.
  SELECT 
    ProductName, SaleDate, SaleAmount,
    SaleAmount * 1.0 / LAG(SaleAmount) OVER (PARTITION BY ProductName ORDER BY SaleDate) AS RatioToPrev
FROM ProductSales;
...
23. Find the Sum of Salaries for the Last 3 Hired Employees
  SELECT 
    ...
    SUM(Salary) OVER (ORDER BY HireDate 
                      ROWS BETWEEN 2 PRECEDING AND CURRENT ROW) AS Last3Sum
FROM Employees1;
