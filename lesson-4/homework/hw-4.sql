...
✅ Using SELECT TOP, OFFSET-FETCH ✅ Using ISNULL and COALESCE 
✅ Order By (ASC, DESC) 
...
Easy-Level Tasks (10)
1. Select the top employees from the Employees table
...
5. Order Products table by Price ascending
...
FROM Products;
10. SELECT DISTINCT on Category and ProductName, ordered by ProductName descending
SELECT DISTINCT Category, ProductName
FROM Products
ORDER BY ProductName DESC;

Medium-Level Tasks (10)
11. Top 10 products ordered by Price descending
SELECT TOP 10 * FROM Products
ORDER BY Price DESC;
12. Return first non-NULL from FirstName or LastName in Employees table
SELECT COALESCE(FirstName, LastName) AS FirstNonNullName, * FROM Employees;
13. Distinct Category and Price from Products table
SELECT DISTINCT Category, Price FROM Products;
14. Employees aged between 30 and 40 OR in Marketing department
SELECT * FROM Employees
WHERE (Age BETWEEN 30 AND 40) OR Department = 'Marketing';
15. Rows 11 to 20 from Employees table, ordered by Salary descending
SELECT * FROM Employees
ORDER BY Salary DESC
OFFSET 10 ROWS FETCH NEXT 10 ROWS ONLY;
16. Products with Price <= 1000 and Stock > 50, sorted by Stock ascending
SELECT * FROM Products
WHERE Price <= 1000 AND Stock > 50
ORDER BY Stock ASC;
17. Products with ProductName containing letter 'e'
SELECT * FROM Products
WHERE ProductName LIKE '%e%';
18. Employees in HR, IT, or Finance
SELECT * FROM Employees
WHERE Department IN ('HR', 'IT', 'Finance');
19. Customers ordered by City ascending, PostalCode descending
SELECT * FROM Customers
ORDER BY City ASC, PostalCode DESC;

Hard-Level Tasks
20. Top 5 products with highest SalesAmount
SELECT TOP (5) * FROM Products
ORDER BY SalesAmount DESC;
21. Combine FirstName and LastName into FullName
SELECT (FirstName + ' ' + LastName) AS FullName, * FROM Employees;
22. DISTINCT Category, ProductName, and Price for products priced above $50
SELECT DISTINCT Category, ProductName, Price FROM Products
WHERE Price > 50;
23. Products whose Price is less than 10% of average price
SELECT * FROM Products
WHERE Price < 0.1 * (SELECT AVG(Price) FROM Products);
24. Employees under 30 in HR or IT
SELECT * FROM Employees
...
SELECT * FROM Employees
WHERE Salary > ALL (ELECT Salary FROM Employees WHERE Department = 'Sales');
27. Orders placed in the last 180 days
SELECT * FROM Orders
WHERE OrderDate BETWEEN DATEADD(DAY, -180, GETDATE()) AND GETDATE();
For Task 8,
GETDATE() gives today’s date in SQL Server.
DATEADD(DAY, -180, GETDATE()) calculates 180 days before today.
