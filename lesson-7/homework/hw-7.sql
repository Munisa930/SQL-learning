...
FROM Employees
WHERE DeptID = 'HR';   -- or DeptName = 'HR' if column exists
...
-- 20. Average price for each product category, filter > 150
SELECT Category,
       AVG(Price) AS AvgPrice
FROM Products
GROUP BY Category
...
-- 21. Total sales per customer, filter > 1500
SELECT CustomerID,
       SUM(SaleAmount) AS TotalSales
FROM Sales
GROUP BY CustomerID
...
-- 22. Total & average salary per department, filter Avg > 65000
SELECT DeptID,
...
-- 23. Total amount of orders with Freight > 50 for each customer,
--    along with their least purchase (MIN freight, may be < 50)
-- (Using TSQL2012 database: Sales.Orders table)
SELECT CustomerID,
       SUM(Freight) AS TotalAmountOver50,
...
-- 24. Total sales & unique products sold in each month-year,
--    filter months with at least 2 distinct products sold
...
-- 25. MIN and MAX order quantity per year (Orders table)
